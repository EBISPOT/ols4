...
use arrow::array::{
    Array, FixedSizeListArray, Float32Array, Float64Array, LargeListArray, LargeStringArray,
...
fn get_string_column<'a>(
    col: &'a dyn Array,
    col_name: &str,
) -> Result<StringColumnRef<'a>, Box<dyn std::error::Error>> {
    if let Some(arr) = col.as_any().downcast_ref::<StringArray>() {
        ...
        Err(format!(
            "{} is not a string column (expected StringArray or LargeStringArray)",
            col_name
        )
        .into())
    }
        ...
        for batch_result in reader {
            ...
            let embedding_list: Box<dyn Fn(usize) -> arrow::array::ArrayRef> =
                ...
                } else if let Some(list) = embedding_col.as_any().downcast_ref::<FixedSizeListArray>()
                {
                    ...
                    return Err(format!(
                        "embedding is not a list column (got {:?})",
                        embedding_col.data_type()
                    )
                    .into());
                };
            ...
            for i in 0..batch.num_rows() {
                ...
                let embedding: Vec<f32> =
                    if let Some(float_array) = embedding_array.as_any().downcast_ref::<Float32Array>()
                    {
                        (0..float_array.len())
                            .map(|j| float_array.value(j))
                            .collect()
                    } else if let Some(float_array) =
                        embedding_array.as_any().downcast_ref::<Float64Array>()
                    {
                        (0..float_array.len())
                            .map(|j| float_array.value(j) as f32)
                            .collect()
                    } else {
                        return Err(format!(
                            "embedding elements are not f32 or f64 (got {:?})",
