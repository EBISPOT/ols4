...
/**
 * Service for collecting ontology loading status from individual rdf2json runs
 * and generating a consolidated report with notifications.
 * 
 * This service is designed to run as a separate step after all rdf2json processes
 * have completed in the nextflow pipeline.
 */
public class ReportingService {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Option configFile = new Option(null, "config", true, "Path to the OLS config JSON file");
        configFile.setRequired(true);
        options.addOption(configFile);

        Option statusDir = new Option(null, "statusDir", true, "Directory containing individual ontology status JSON files");
        statusDir.setRequired(true);
        options.addOption(statusDir);

        Option reportFile = new Option(null, "reportFile", true, "Output file for the consolidated report");
        ...
        try {
            ...
            formatter.printHelp("reporting", options);
            System.exit(1);
            return;
        }

        String configFilePath = cmd.getOptionValue("config");
        String statusDirPath = cmd.getOptionValue("statusDir");
        String reportFilePath = cmd.getOptionValue("reportFile");
        boolean bSendNotifications = cmd.hasOption("sendNotifications");

        ReportingService service = new ReportingService(configFilePath);
        service.collectStatuses(statusDirPath);
        service.generateReportAndNotify(reportFilePath, bSendNotifications);
    }
    ...
    public ReportingService(String configFilePath) {
    ...
    /**
     * Collect all status JSON files from the status directory.
     */
    public void collectStatuses(String statusDirPath) throws IOException {
        logger.info("Collecting status files from: {}", statusDirPath);
        
        Path statusDir = Paths.get(statusDirPath);
        if (!Files.exists(statusDir)) {
            logger.warn("Status directory does not exist: {}", statusDirPath);
            return;
        }

        Gson gson = new Gson();
        
        Files.list(statusDir)
            .filter(path -> path.toString().endsWith(".status.json"))
            .forEach(path -> {
                try {
                    logger.debug("Reading status file: {}", path);
                    OntologyLoadStatus status = gson.fromJson(
                        new FileReader(path.toFile()), 
                        OntologyLoadStatus.class
                    );
                    loadStatuses.put(status.getOntologyId().toLowerCase(), status);
                } catch (Exception e) {
                    logger.error("Failed to read status file {}: {}", path, e.getMessage());
                }
            });
        
        logger.info("Collected {} ontology statuses", loadStatuses.size());
    }
